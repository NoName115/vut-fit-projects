...
**/
	...
	tStackP *tempStack = malloc(sizeof(tStackP));
...
**/
	...
	while (!SEmptyP(tempStack) || temp != NULL)
	{
		if (temp != NULL)
		{
			Leftmost_Inorder(temp, tempStack);
		}
...
**/
	tBTNodePtr temp = RootPtr;
	tBTNodePtr lastVisited = NULL;

	tStackP *tempStackP = malloc(sizeof(tStackP));
	...
	tStackB *tempStackB = malloc(sizeof(tStackB));
	...
	while (!SEmptyP(tempStackP) || temp != NULL)
	{
		if (temp != NULL)
		{
			Leftmost_Postorder(temp, tempStackP, tempStackB);
		}

		tBTNodePtr peekNode = tempStackP->a[tempStackP->top];
		if (peekNode->RPtr != NULL && lastVisited != peekNode->RPtr)
		{
			temp = peekNode->RPtr;
		}
		else
		{
			BTWorkOut(peekNode);
			lastVisited = STopPopP(tempStackP);
			temp = NULL;
		}
	}
...
**/
	...
	while (!SEmptyP(tempStack) || temp != NULL)
	{
		...
		tBTNodePtr freeTemp = STopPopP(tempStack);
		temp = freeTemp->RPtr;
		free(freeTemp);
	}
