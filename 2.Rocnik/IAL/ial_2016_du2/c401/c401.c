...
**/
	...
	else
	{
		...
		{
			leftOutput = BSTSearch(RootPtr->LPtr, K, Content);
		}

		if (RootPtr->RPtr != NULL && leftOutput == 0)
		{
			rightOutput = BSTSearch(RootPtr->RPtr, K, Content);
		}
...
**/
	...
	else
	{
		...
		if ((*RootPtr)->LPtr != NULL)
		{
			tBSTNodePtr tempNode = (*RootPtr);
			(*RootPtr) = tempNode->LPtr;
			free(tempNode);
		}
		else
		{
...
**/
	...
	if ((*RootPtr)->Key == K)
	{
		...
		else if ((*RootPtr)->LPtr == NULL) //Pravy potomok
		{
			tBSTNodePtr tempNode = (*RootPtr);
			(*RootPtr) = (*RootPtr)->RPtr;
			free(tempNode);
		}
		else if ((*RootPtr)->RPtr == NULL) //Lavy potomok
		{
			tBSTNodePtr tempNode = (*RootPtr);
			(*RootPtr) = (*RootPtr)->LPtr;
			free(tempNode);
		}
		else //Obaja potomkovia
		{
	...
	{
		if (K < (*RootPtr)->Key && (*RootPtr)->LPtr != NULL)
		{
			BSTDelete(&(*RootPtr)->LPtr, K);
		}
		if (K > (*RootPtr)->Key && (*RootPtr)->RPtr != NULL)
		{
